...
public class ConfigScreen extends ScreenMapper implements IGui {
	...
	private final WidgetShorterSlider sliderTrackRenderDistance;
	...
	public ConfigScreen() {
		...
		buttonHideAllTrains = new Button(0, 0, 0, BUTTON_HEIGHT, Text.literal(""), button -> {
			hideAllTrains = Config.hideAllTrains(!hideAllTrains);
			setButtonText(button, hideAllTrains);
		});
		buttonHideAllTracks = new Button(0, 0, 0, BUTTON_HEIGHT, Text.literal(""), button -> {
			hideAllTracks = Config.hideAllTracks(!hideAllTracks);
			setButtonText(button, hideAllTracks);
		});
		buttonHideTrainWhenRiding = new Button(0, 0, 0, BUTTON_HEIGHT, Text.literal(""), button -> {
		...
		sliderTrackRenderDistance = new WidgetShorterSlider(0, 0, Config.TRACK_RENDER_DISTANCE_COUNT - 1, num -> String.format("%d%%", (num + 1) * 100 / Config.TRACK_RENDER_DISTANCE_COUNT), null);
	...
	protected void init() {
		...
		IDrawing.setPositionAndWidth(sliderTrackRenderDistance, width - SQUARE_SIZE - BUTTON_WIDTH, BUTTON_HEIGHT * (i++) + SQUARE_SIZE, BUTTON_WIDTH - TEXT_PADDING - font.width("100%"));
		...
		sliderTrackRenderDistance.setHeight(BUTTON_HEIGHT);
		sliderTrackRenderDistance.setValue(Config.trackRenderDistance());
		...
		addDrawableChild(sliderTrackRenderDistance);
	...
	public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
		try {
			...
			drawString(matrices, font, Text.translatable("options.mtr.hide_all_vehicles"), SQUARE_SIZE, BUTTON_HEIGHT * (i++) + yStart1, ARGB_WHITE);
			drawString(matrices, font, Text.translatable("options.mtr.hide_all_tracks"), SQUARE_SIZE, BUTTON_HEIGHT * (i++) + yStart1, ARGB_WHITE);
			drawString(matrices, font, Text.translatable("options.mtr.hide_train_when_riding"), SQUARE_SIZE, BUTTON_HEIGHT * (i++) + yStart1, ARGB_WHITE);
			...
			drawString(matrices, font, Text.translatable("options.mtr.track_rendering_distance"), SQUARE_SIZE, BUTTON_HEIGHT * (i++) + yStart1, ARGB_WHITE);
	...
	public void onClose() {
		...
		Config.setTrackRenderDistance(sliderTrackRenderDistance.getIntValue());
