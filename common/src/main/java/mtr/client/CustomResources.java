			...
			try {
				jsonConfig.get(CUSTOM_TRAINS_KEY).getAsJsonObject().entrySet().forEach(entry -> {
					try {
						...
						final JonTrainSound jonSoundOrDefault = baseTrainProperties.sound instanceof JonTrainSound ? (JonTrainSound) baseTrainProperties.sound : new JonTrainSound("", new JonTrainSound.JonTrainSoundConfig(null, null, 0, 0.5F, false, false));
						...
						final String hornSoundBaseId = getOrDefault(jsonObject, CUSTOM_TRAINS_HORN_SOUND_BASE_ID, jonSoundOrDefault.config.hornSoundBaseId, JsonElement::getAsString);
						...
						if (!baseTrainProperties.baseTrainType.isEmpty()) {
							...
							final JonTrainSound.JonTrainSoundConfig soundConfig = useBveSound ? null : new JonTrainSound.JonTrainSoundConfig(doorSoundBaseId, hornSoundBaseId, speedSoundCount, doorCloseSoundTime, accelSoundAtCoast, constPlaybackSpeed);
