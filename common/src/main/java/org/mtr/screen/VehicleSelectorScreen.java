...
public class VehicleSelectorScreen extends DashboardListSelectorScreen implements Icons {
	...
	private final ButtonWidget buttonDuplicateVehicleCars;
	...
	public VehicleSelectorScreen(Siding siding, Screen previousScreen) {
		super(getVehicleList(siding.getTransportMode()), getSelectedIds(siding), false, true, previousScreen);
		buttonDuplicateVehicleCars = ButtonWidget.builder(TranslationProvider.GUI_MTR_DUPLICATE_VEHICLE_CARS.getMutableText(), button -> {
	...
	public void renderAdditional(DrawContext context, int mouseX, int mouseY, float delta) {
		final int spareSpace = Math.max(0, width - SQUARE_SIZE * 4 - PANEL_WIDTH * 2);
		context.drawCenteredTextWithShadow(MinecraftClient.getInstance().textRenderer, TranslationProvider.GUI_MTR_AVAILABLE.getMutableText(), SQUARE_SIZE * 2 + spareSpace + PANEL_WIDTH / 2, SQUARE_SIZE, ARGB_WHITE);
		context.drawCenteredTextWithShadow(MinecraftClient.getInstance().textRenderer, TranslationProvider.GUI_MTR_SELECTED.getMutableText(), SQUARE_SIZE * 3 + spareSpace + PANEL_WIDTH * 3 / 2, SQUARE_SIZE, ARGB_WHITE);
		...
		CustomResourceLoader.getVehicleByIndex(siding.getTransportMode(), index, vehicleResource -> {
			int y = SQUARE_SIZE;
			y = drawWrappedText(context, vehicleResource.getName(), y, ARGB_WHITE);
			y = drawWrappedText(context, TranslationProvider.GUI_MTR_VEHICLE_LENGTH.getMutableText(vehicleResource.getLength()), y, ARGB_WHITE);
			final String description = vehicleResource.getDescription().getString();
			if (!description.isEmpty()) {
				for (final String text : description.split("[|\n]")) {
					y = drawWrappedText(context, Text.literal(text), y, ARGB_LIGHT_GRAY);
				}
			}
	...
	protected void updateList() {
		...
		for (final DashboardListItem data : allData) {
			if (data instanceof VehicleForList vehicleForList) {
	...
	private int drawWrappedText(DrawContext context, MutableText component, int y, int color) {
		final List<OrderedText> splitText = textRenderer.wrapLines(component, Math.max(0, width - SQUARE_SIZE * 4 - PANEL_WIDTH * 2));
		int newY = y;
		for (final OrderedText formattedCharSequence : splitText) {
			...
			if (nextY > height - SQUARE_SIZE - TEXT_HEIGHT) {
				context.drawText(textRenderer, "...", SQUARE_SIZE, newY, color, false);
				return height;
			} else {
				context.drawText(textRenderer, formattedCharSequence, SQUARE_SIZE, newY, color, false);
			}
			newY = nextY;
		}
