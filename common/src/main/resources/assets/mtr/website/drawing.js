...
function drawMap(container, data) {
	...
	for (const positionKey in positions) {
		...
		if (SETTINGS.routeType == -1 || (typeof route !== "undefined" && route["type"] === types[SETTINGS.routeType])) {
	...
	for (const routeKey in routes) {
		...
		if (SETTINGS.routeType == -1 || routeType === types[SETTINGS.routeType]) {
	...
	for (const stationId in blobs) {
		...
		if (SETTINGS.routeType == -1 || blob[types[SETTINGS.routeType]]) {
