		...
		if (renderColors || texture != null) {
			rail.render((x1, z1, x2, z2, x3, z3, x4, z4, y1, y2) -> {
				...
				if (renderColors && ClientData.instance.oneWayRails.contains(rail)) {
					RenderTrains.scheduleRender(new Identifier(MTR.MOD_ID, "textures/block/one_way_rail_arrow.png"), false, RenderTrains.QueuedRenderLayer.EXTERIOR, graphicsHolder -> {
						final Vector3d playerOffset = RenderTrains.getPlayerOffset();
