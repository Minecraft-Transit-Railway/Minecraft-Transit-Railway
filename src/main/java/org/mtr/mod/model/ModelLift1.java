...
public class ModelLift1 extends ModelTrainBase {
			...
			for (int j = 0; j <= depth; j++) {
				...
				switch (renderStage) {
					...
					case EXTERIOR:
						...
						if (edge1X && !edge2X && !edge1Z && !edge2Z) {
							mainEdgePiece.render(graphicsHolder, x, z - 4, (float) -Math.PI / 2, light, OverlayTexture.DEFAULT_UV);
							mainEdgePiece.render(graphicsHolder, x, z + 4, (float) -Math.PI / 2, light, OverlayTexture.DEFAULT_UV);
							...
							mainEdgeCeilingPiece.render(graphicsHolder, x, heightOffset, z - 4, (float) -Math.PI / 2, light, OverlayTexture.DEFAULT_UV);
							mainEdgeCeilingPiece.render(graphicsHolder, x, heightOffset, z + 4, (float) -Math.PI / 2, light, OverlayTexture.DEFAULT_UV);
						}
						if (!edge1X && edge2X && !edge1Z && !edge2Z) {
							mainEdgePiece.render(graphicsHolder, x, z - 4, (float) Math.PI / 2, light, OverlayTexture.DEFAULT_UV);
						...
						if (edge1X && !edge2X && !edge1Z && edge2Z && (width > 2 || !isDoubleSided)) {
							mainCornerPiece.render(graphicsHolder, x, z, 0, light, OverlayTexture.DEFAULT_UV);
							// TODO edge exterior pieces
							if (renderStage == RenderStage.INTERIOR) {
								renderWall(mainCornerWallPiece, graphicsHolder, x, z, 0, light);
							}
							mainCornerCeilingPiece.render(graphicsHolder, x, heightOffset, z, 0, light, OverlayTexture.DEFAULT_UV);
						}
						if (!edge1X && edge2X && !edge1Z && edge2Z && (width > 2 || !isDoubleSided)) {
							mainCornerPiece.render(graphicsHolder, x, z, (float) Math.PI / 2, light, OverlayTexture.DEFAULT_UV);
							// TODO edge exterior pieces
							if (renderStage == RenderStage.INTERIOR) {
								renderWall(mainCornerWallPiece, graphicsHolder, x, z, (float) Math.PI / 2, light);
							}
							mainCornerCeilingPiece.render(graphicsHolder, x, heightOffset, z, (float) Math.PI / 2, light, OverlayTexture.DEFAULT_UV);
						}
						...
						if (!edge1X && edge2X && edge1Z && !edge2Z && width > 2) {
							mainCornerPiece.render(graphicsHolder, x, z, (float) Math.PI, light, OverlayTexture.DEFAULT_UV);
							// TODO edge exterior pieces
							if (renderStage == RenderStage.INTERIOR) {
								renderWall(mainCornerWallPiece, graphicsHolder, x, z, (float) Math.PI, light);
							}
							mainCornerCeilingPiece.render(graphicsHolder, x, heightOffset, z, (float) Math.PI, light, OverlayTexture.DEFAULT_UV);
						}
	...
	private void renderWall(ModelPartExtension model, GraphicsHolder graphicsHolder, float x, float z, float rotateY, int light) {
		for (int i = 0; i < heightCount; i++) {
			model.render(graphicsHolder, x, -i * 8, z, rotateY, light, OverlayTexture.DEFAULT_UV);
		}
	}
