...
public class RenderAPGGlass extends RenderRouteBase<BlockAPGGlass.TileEntityAPGGlass> implements IGui, IBlock {
	...
	protected void renderAdditional(List<RenderingInstruction> renderingInstructions, RouteRenderer routeRenderer, BlockState state, Direction facing, int light) {
		if (IBlock.getStatePropertySafe(state, HALF) == DoubleBlockHalf.UPPER && IBlock.getStatePropertySafe(state, SIDE_EXTENDED) != EnumSide.SINGLE) {
			...
			RenderingInstruction.addVertexConsumer(renderingInstructions, vertexConsumers -> vertexConsumers.getBuffer(MoreRenderLayers.getExterior(new Identifier("mtr:textures/block/white.png"))));
			RenderingInstruction.addDrawTexture(renderingInstructions, isLeft ? getSidePadding() : 0, getTopPadding(), SMALL_OFFSET * 2, isRight ? getSidePadding() : 1, COLOR_STRIP_END, SMALL_OFFSET * 2, facing, ARGB_WHITE, light);
			RenderingInstruction.addDrawTexture(renderingInstructions, isRight ? getSidePadding() : 1, getTopPadding(), 0.125F - SMALL_OFFSET * 2, isLeft ? getSidePadding() : 0, COLOR_STRIP_END, 0.125F - SMALL_OFFSET * 2, facing, ARGB_WHITE, light);
		}
	}
}
