...
public class ScheduleScreen extends Screen implements IGui, IPacket {
	...
	public ScheduleScreen(Route route, DashboardScreen dashboardScreen) {
		...
		for (int i = 0; i < Route.HOURS_IN_DAY; i++) {
			final int index = i;
			sliders[i] = new WidgetShorterSlider(sliderX, SLIDER_WIDTH, MAX_TRAINS_PER_HOUR * 2, value -> this.route.setFrequencies(value, index, packet -> PacketTrainDataGuiClient.sendUpdate(PACKET_UPDATE_ROUTE, packet)), ScheduleScreen::getSliderString);
		}
