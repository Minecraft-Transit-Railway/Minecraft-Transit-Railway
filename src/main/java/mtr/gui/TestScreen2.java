...
public class TestScreen2 extends Screen implements IGui {
	...
	private final WidgetShorterSlider[] sliders = new WidgetShorterSlider[HOURS_IN_DAY];
	...
	public TestScreen2(BlockPos spawnerPos) {
		...
		for (int i = 0; i < sliders.length; i++) {
			sliders[i] = new WidgetShorterSlider(sliderX, SLIDER_WIDTH, MAX_TRAINS_PER_HOUR * 2, TestScreen2::getSliderString);
		}
	...
	public void tick() {
		...
		routeList.setData(trainSpawner.routeIds.stream().map(routeId -> RailwayData.getRouteById(ScreenBase.GuiBase.routes, routeId)).collect(Collectors.toList()), false, false, true, false, true);
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		if (addingRoute || addingTrain) {
			...
			final int lineHeight = Math.min(SQUARE_SIZE, (height - SQUARE_SIZE) / HOURS_IN_DAY);
	...
	private void onAddingRoute() {
		addNewList.setData(ScreenBase.GuiBase.routes, false, false, false, true, false);
	...
	private TrainSpawner getTrainSpawner() {
		return ScreenBase.GuiBase.trainSpawners.stream().filter(trainSpawner -> trainSpawner.pos.equals(spawnerPos)).findFirst().orElse(null);
	}
