...
public class MathTools {
	...
	public static double angleBetweenPoints(double xCenter, double zCenter, double x2, double z2) {
		double distance = distanceBetweenPoints(xCenter, zCenter, x2, z2);
		if (distance == 0)
			return 0;
	...
	public static double[] circleIntersection(double h1, double k1, double r1, double h2, double k2, double r2) {
		double d2 = Math.pow(h2 - h1, 2) + Math.pow(k2 - k1, 2);
		double K = 0.25 * Math.sqrt((Math.pow(r1 + r2, 2) - d2) * (d2 - Math.pow(r1 - r2, 2)));
		double x1 = 0.5 * (h2 + h1) + 0.5 * (h2 - h1) * (Math.pow(r1, 2) - Math.pow(r2, 2)) / d2
				+ 2 * (k2 - k1) * K / d2;
		double x2 = 0.5 * (h2 + h1) + 0.5 * (h2 - h1) * (Math.pow(r1, 2) - Math.pow(r2, 2)) / d2
				- 2 * (k2 - k1) * K / d2;
		double y1 = 0.5 * (k2 + k1) + 0.5 * (k2 - k1) * (Math.pow(r1, 2) - Math.pow(r2, 2)) / d2
				- 2 * (h2 - h1) * K / d2;
		double y2 = 0.5 * (k2 + k1) + 0.5 * (k2 - k1) * (Math.pow(r1, 2) - Math.pow(r2, 2)) / d2
				+ 2 * (h2 - h1) * K / d2;
		double[] a = { x1, y1, x2, y2 };
		return a;
	}

	public static ModelRenderer part(ModelBase model, int texOffsetX, int texOffsetZ, float offX, float offY,
			float offZ, int width, int height, int depth, float rotX, float rotY, float rotZ, int texSizeX,
			int texSizeZ) {
		ModelRenderer part = new ModelRenderer(model, texOffsetX, texOffsetZ);
