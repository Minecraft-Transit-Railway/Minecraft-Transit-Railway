...
export class EditVehiclePropertiesDialog {
	...
	onSave() {
		...
		const defaultVehicle = CREATE_VEHICLE_RESOURCE();
		this.vehicleResource.id = newData.id ?? defaultVehicle.id;
		this.vehicleResource.name = newData.name ?? defaultVehicle.name;
		this.vehicleResource.color = newData.color?.substring(1).toUpperCase() ?? defaultVehicle.color;
		this.vehicleResource.transportMode = newData.transportMode ?? defaultVehicle.transportMode;
		this.vehicleResource.description = newData.description ?? defaultVehicle.description;
		this.vehicleResource.wikipediaArticle = newData.wikipediaArticle ?? defaultVehicle.wikipediaArticle;
		this.vehicleResource.length = Math.max(0.5, newData.length ?? defaultVehicle.length);
		this.vehicleResource.width = Math.max(0.5, newData.width ?? defaultVehicle.width);
		this.vehicleResource.bogie1Position = (newData.hasTwoBogies ? newData.bogie1Position : newData.bogiePosition) ?? defaultVehicle.bogie1Position;
		this.vehicleResource.bogie2Position = (newData.hasTwoBogies ? newData.bogie2Position : newData.bogiePosition) ?? defaultVehicle.bogie2Position;
		this.vehicleResource.couplingPadding1 = Math.max(0, newData.couplingPadding1 ?? defaultVehicle.couplingPadding1);
		this.vehicleResource.couplingPadding2 = Math.max(0, newData.couplingPadding2 ?? defaultVehicle.couplingPadding2);
		this.vehicleResource.hasGangway1 = newData.hasGangway1 ?? defaultVehicle.hasGangway1;
		this.vehicleResource.hasGangway2 = newData.hasGangway2 ?? defaultVehicle.hasGangway2;
		this.vehicleResource.hasBarrier1 = newData.hasBarrier1 ?? defaultVehicle.hasBarrier1;
		this.vehicleResource.hasBarrier2 = newData.hasBarrier2 ?? defaultVehicle.hasBarrier2;
		this.vehicleResource.bveSoundBaseResource = (newData.soundType === "bve" ? newData.bveSoundBaseResource : "") ?? defaultVehicle.bveSoundBaseResource;
		this.vehicleResource.legacySpeedSoundBaseResource = (newData.soundType === "bve" ? "" : newData.legacySpeedSoundBaseResource) ?? defaultVehicle.legacySpeedSoundBaseResource;
		this.vehicleResource.legacySpeedSoundCount = Math.max(1, Math.round((newData.soundType === "bve" ? 0 : newData.legacySpeedSoundCount) ?? defaultVehicle.legacySpeedSoundCount));
		this.vehicleResource.legacyUseAccelerationSoundsWhenCoasting = (newData.soundType === "bve" ? false : newData.legacyUseAccelerationSoundsWhenCoasting) ?? defaultVehicle.legacyUseAccelerationSoundsWhenCoasting;
		this.vehicleResource.legacyConstantPlaybackSpeed = (newData.soundType === "bve" ? false : newData.legacyConstantPlaybackSpeed) ?? defaultVehicle.legacyConstantPlaybackSpeed;
		this.vehicleResource.legacyDoorSoundBaseResource = (newData.soundType === "bve" ? "" : newData.legacyDoorSoundBaseResource) ?? defaultVehicle.legacyDoorSoundBaseResource;
		this.vehicleResource.legacyDoorCloseSoundTime = Math.max(0, Math.min(1, (newData.soundType === "bve" ? 0 : newData.legacyDoorCloseSoundTime) ?? defaultVehicle.legacyDoorCloseSoundTime));
