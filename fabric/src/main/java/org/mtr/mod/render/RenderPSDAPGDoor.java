	...
	public void render(T entity, float tickDelta, GraphicsHolder graphicsHolder, int light, int overlay) {
		...
		switch (type) {
			...
			case 1:
				...
				if (half && !unlocked) {
					RenderTrains.scheduleRender(new Identifier(Init.MOD_ID, "textures/block/sign/door_not_in_use.png"), false, RenderTrains.QueuedRenderLayer.EXTERIOR, graphicsHolderNew -> {
						storedMatrixTransformations.transform(graphicsHolderNew);
						if (end) {
