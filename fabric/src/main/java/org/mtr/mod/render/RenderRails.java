		...
		for (final String style : rail.getStyles()) {
			...
			if (newStyle.equals(CustomResourceLoader.DEFAULT_RAIL_ID)) {
				...
				CustomResourceLoader.getRailById(RailResource.getIdWithoutDirection(newStyle), railResource -> renderWithinRenderDistance(rail, (blockPos, x1, z1, x2, z2, x3, z3, x4, z4, y1, y2) -> {
					...
					final float rollDegrees = (float) ((x1 * z1) % 10) / 100;
					if (!railResource.queueGpu((x1 + x3) / 2, (y1 + y2) / 2 + railResource.getModelYOffset(), (z1 + z3) / 2, yaw, pitch, flip, rollDegrees, light, true)) {
						final StoredMatrixTransformations storedMatrixTransformations = RailResource.createStoredMatrixTransformations((x1 + x3) / 2, (y1 + y2) / 2 + railResource.getModelYOffset(), (z1 + z3) / 2, yaw, pitch, flip, rollDegrees, true);
