...
public class SidingScreen extends SavedRailScreenBase<Siding, Depot> implements Icons {
	...
	protected void init2() {
		...
		sliderBrakingPowerConstant.setY2(SQUARE_SIZE * 5 + TEXT_FIELD_PADDING * 2);
	...
	public void render(GraphicsHolder graphicsHolder, int mouseX, int mouseY, float delta) {
		...
		if (showScheduleControls) {
			...
			graphicsHolder.drawText(BRAKING_POWER_CONSTANT_TEXT, SQUARE_SIZE, SQUARE_SIZE * 5 + TEXT_FIELD_PADDING * 2 + TEXT_PADDING, ARGB_WHITE, false, MAX_LIGHT_GLOWING);
			if (buttonIsManual.isChecked2()) {
				graphicsHolder.drawText(MAX_MANUAL_SPEED, SQUARE_SIZE, SQUARE_SIZE * 8 + TEXT_FIELD_PADDING * 2 + TEXT_PADDING, ARGB_WHITE, false, MAX_LIGHT_GLOWING);
				graphicsHolder.drawText(MANUAL_TO_AUTOMATIC_TIME, SQUARE_SIZE, SQUARE_SIZE * 9 + TEXT_FIELD_PADDING * 2 + TEXT_PADDING, ARGB_WHITE, false, MAX_LIGHT_GLOWING);
			}
		}
	}
