...
public class EditDepotScreen extends EditNameColorScreenBase<Depot> {
	...
	public EditDepotScreen(Depot depot, TransportMode transportMode) {
		super(depot, TranslationProvider.GUI_MTR_DEPOT_NAME, TranslationProvider.GUI_MTR_DEPOT_COLOR);
		...
		buttonEditInstructions = new ButtonWidgetExtension(0, 0, 0, SQUARE_SIZE, TranslationProvider.GUI_MTR_EDIT_INSTRUCTIONS.getMutableText(), button -> {
			...
			MinecraftClient.getInstance().openScreen(new Screen(new DashboardListSelectorScreen(new ObjectImmutableList<>(routes), data.getRouteIds(), false, true).withPreviousScreen(this)));
		});
