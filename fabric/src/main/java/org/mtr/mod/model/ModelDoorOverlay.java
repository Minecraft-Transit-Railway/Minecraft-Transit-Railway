...
public class ModelDoorOverlay extends EntityModelExtension<EntityAbstractMapping> {
	...
	public ModelDoorOverlay(int doorMax, float angle, int pivotY, String overlayLeftTextureName, String overlayRightTextureName, boolean renderLeft, boolean renderRight) {
		...
		doorOverlayTextureLeft = new Identifier(Init.MOD_ID, "textures/block/sign/" + overlayLeftTextureName);
		doorOverlayTextureRight = new Identifier(Init.MOD_ID, "textures/block/sign/" + overlayRightTextureName);
	...
	public void render(StoredMatrixTransformations storedMatrixTransformations, ModelTrainBase.RenderStage renderStage, int light, int position, float doorLeftX, float doorRightX, float doorLeftZ, float doorRightZ, boolean lightsOn) {
		switch (renderStage) {
			case INTERIOR:
				...
				if (renderRight) {
					RenderTrains.scheduleRender(doorOverlayTextureLeft, false, lightsOn ? RenderTrains.QueuedRenderLayer.INTERIOR : RenderTrains.QueuedRenderLayer.EXTERIOR, (graphicsHolder, offset) -> {
					...
					RenderTrains.scheduleRender(doorOverlayTextureRight, false, lightsOn ? RenderTrains.QueuedRenderLayer.INTERIOR : RenderTrains.QueuedRenderLayer.EXTERIOR, (graphicsHolder, offset) -> {
						storedMatrixTransformations.transform(graphicsHolder, offset);
			...
			case EXTERIOR:
				if (renderRight) {
					RenderTrains.scheduleRender(doorOverlayTextureLeft, false, RenderTrains.QueuedRenderLayer.EXTERIOR, (graphicsHolder, offset) -> {
						storedMatrixTransformations.transform(graphicsHolder, offset);
						ModelTrainBase.renderOnce(door_left_overlay_exterior, graphicsHolder, light / 4 * 3, doorRightX, position + doorRightZ);
						graphicsHolder.pop();
					});
					RenderTrains.scheduleRender(doorOverlayTextureRight, false, RenderTrains.QueuedRenderLayer.EXTERIOR, (graphicsHolder, offset) -> {
						storedMatrixTransformations.transform(graphicsHolder, offset);
